...
def path_module(path: str) -> str | None:
    ...
    if path.startswith("/api/excel"):
        return "excel"
    if path.startswith("/api/recursos") or path.startswith("/api/tardiness"):
        return "recursos"
    if path.startswith("/api/offline"):
        return "offline"
    if path.startswith("/api/employee-schedules/import"):
        return None
    if path.startswith("/api/users") or path.startswith("/api/employee-schedules") or path.startswith("/api/absences"):
        return "employees"
